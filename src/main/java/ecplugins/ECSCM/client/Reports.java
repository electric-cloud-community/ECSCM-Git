    ...
    extends ComponentBase
{
    ...
    @Override public Widget doInit()
    {
        ...
        Widget htmlH1 = new HTML("<h1>Git Changelog</h1>");

        vPanel.add(htmlH1);

        Widget htmlLabel = new HTML(
                "<p><b>Git changelogs associated with the ElectricCommander job:</b></p>");
            ...
            final FormTable formTable)
    {
        ...
        if (getLog().isDebugEnabled()) {
            getLog().debug(
                "Git Reports doInit: setting up getProperties command request");
        }
        ...
        if (getLog().isDebugEnabled()) {
            getLog().debug("Git Reports doInit: Issuing CloudBees CD request: "
            ...
            FormTable     form)
    {
        ...
        for (Property p : response.getProperties()
            ...
            HTML htmlH1 = new HTML("<h3>" + p.getName() + "</h3> <pre>"
                        + p.getValue().replaceAll("<", "&lt;").replaceAll(">", "&gt;") + "</pre>");
